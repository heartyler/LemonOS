...
$Classes = Join-Path $Root "build\test-proxy-boards-gate"
$VelocityCandidates = @(
...
$Harness = Join-Path $Root "tools\java\dev\lemonos\proxy\ProxyBoardsGateHarness.java"
...
if ($LASTEXITCODE -ne 0) { throw "Proxy Boards gate harness compilation failed." }
& (Join-Path $JdkRoot "bin\java.exe") -cp "$Classes;$VelocityJar" dev.lemonos.proxy.ProxyBoardsGateHarness
if ($LASTEXITCODE -ne 0) { throw "Proxy Boards gate behavioral contract failed." }
Write-Host "Proxy Boards canonical gate contract passed."
