...
final class BackendStayedCloseDisplayService {
    BackendDisplayModel model(BackendDisplayConfig config, String configPath, List<Rank> ranks) {
        ...
        int top = config.intValue(path + ".top", 5, 1, 10);
        double subtitleOffsetY = config.doubleValue(path + ".display.subtitle-offset-y", -0.10, -4.0, 4.0);
        double rowStartOffsetY = config.doubleValue(path + ".display.row-start-offset-y", -0.34, -4.0, 4.0);
        double rowGap = config.doubleValue(path + ".display.row-gap", -0.13, -2.0, 2.0);
        double nameOffsetZ = config.doubleValue(path + ".display.name-offset-z", -0.32, -8.0, 8.0);
        double valueOffsetZ = config.doubleValue(path + ".display.value-offset-z", path + ".display.time-offset-z", 0.48, -8.0, 8.0);
        boolean bedrockEnabled = config.booleanValue(path + ".display.bedrock.enabled", false);
        double titleOffsetX = config.doubleValue(path + ".display.title-offset-x", 0.0, -8.0, 8.0);
        double titleOffsetY = config.doubleValue(path + ".display.title-offset-y", 0.0, -4.0, 4.0);
        double titleOffsetZ = config.doubleValue(path + ".display.title-offset-z", 0.0, -8.0, 8.0);
        String title = config.stringValue(path + ".title", "Stayclose").trim();
        String subtitle = config.stringValue(path + ".subtitle", "where small steps stay.").trim();
        String bottomLine = config.stringValue(path + ".bottom-line", "time spent here.").trim();
        ...
        double bottomOffsetY = config.doubleValue(path + ".display.bottom-offset-y", -1.02, -4.0, 4.0);
        double bottomOffsetZ = config.doubleValue(path + ".display.bottom-offset-z", 0.0, -8.0, 8.0);
        ...
        if (bedrockEnabled) {
            double bedrockTitleOffsetX = config.doubleValue(path + ".display.bedrock.title-offset-x", 0.0, -8.0, 8.0);
            double bedrockTitleOffsetY = config.doubleValue(path + ".display.bedrock.title-offset-y", 0.0, -4.0, 4.0);
            double bedrockTitleOffsetZ = config.doubleValue(path + ".display.bedrock.title-offset-z", 0.0, -8.0, 8.0);
            double bedrockSubtitleOffsetX = config.doubleValue(path + ".display.bedrock.subtitle-offset-x", 0.0, -8.0, 8.0);
            double bedrockSubtitleOffsetY = config.doubleValue(path + ".display.bedrock.subtitle-offset-y", -0.10, -4.0, 4.0);
            double bedrockSubtitleOffsetZ = config.doubleValue(path + ".display.bedrock.subtitle-offset-z", 0.0, -8.0, 8.0);
            double bedrockBottomOffsetX = config.doubleValue(path + ".display.bedrock.bottom-offset-x", 0.0, -8.0, 8.0);
            double bedrockBottomOffsetY = config.doubleValue(path + ".display.bedrock.bottom-offset-y", -1.02, -4.0, 4.0);
            double bedrockBottomOffsetZ = config.doubleValue(path + ".display.bedrock.bottom-offset-z", 0.0, -8.0, 8.0);
        ...
        for (int i = 0; i < top; ++i) {
            ...
            String name = rank == null ? "" : BackendDisplayText.fitName(rank.name(), config.intValue(path + ".name-width", 12, 4, 16));
            ...
            if (bedrockEnabled) {
                int bedrockNameWidth = config.intValue(path + ".display.bedrock.name-width", 12, 4, 24);
                double bedrockRowStartOffsetX = config.doubleValue(path + ".display.bedrock.row-start-offset-x", 0.0, -8.0, 8.0);
                double bedrockRowStartOffsetY = config.doubleValue(path + ".display.bedrock.row-start-offset-y", -0.34, -4.0, 4.0);
                double bedrockRowStartOffsetZ = config.doubleValue(path + ".display.bedrock.row-start-offset-z", 0.0, -8.0, 8.0);
                double bedrockRowGap = config.doubleValue(path + ".display.bedrock.row-gap", -0.16, -2.0, 2.0);
